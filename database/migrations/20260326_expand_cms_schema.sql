...
ALTER TABLE users
    ADD COLUMN IF NOT EXISTS full_name VARCHAR(120) DEFAULT NULL AFTER email,
    ADD COLUMN IF NOT EXISTS role ENUM('admin', 'editor') NOT NULL DEFAULT 'admin' AFTER full_name,
    ADD COLUMN IF NOT EXISTS avatar_url VARCHAR(500) DEFAULT NULL AFTER role,
...
ALTER TABLE projects
    ...
    ADD COLUMN IF NOT EXISTS project_category_id SMALLINT DEFAULT NULL AFTER id,
    ADD COLUMN IF NOT EXISTS slug VARCHAR(180) DEFAULT NULL AFTER project_category_id,
    ADD COLUMN IF NOT EXISTS size_value DECIMAL(10,2) DEFAULT NULL AFTER size,
    ADD COLUMN IF NOT EXISTS size_unit VARCHAR(20) NOT NULL DEFAULT 'm2' AFTER size_value,
    ADD COLUMN IF NOT EXISTS image_public_id VARCHAR(255) DEFAULT NULL AFTER image_url,
    ADD COLUMN IF NOT EXISTS short_description VARCHAR(255) DEFAULT NULL AFTER description,
    ADD COLUMN IF NOT EXISTS style_label VARCHAR(100) DEFAULT NULL AFTER short_description,
    ADD COLUMN IF NOT EXISTS client_name VARCHAR(120) DEFAULT NULL AFTER style_label,
    ADD COLUMN IF NOT EXISTS completion_year YEAR DEFAULT NULL AFTER client_name,
    ADD COLUMN IF NOT EXISTS status ENUM('draft', 'published', 'archived') NOT NULL DEFAULT 'published' AFTER completion_year,
    ADD COLUMN IF NOT EXISTS featured_order SMALLINT DEFAULT NULL AFTER is_featured,
    ADD COLUMN IF NOT EXISTS sort_order SMALLINT NOT NULL DEFAULT 0 AFTER featured_order,
    ADD COLUMN IF NOT EXISTS published_at DATETIME DEFAULT NULL AFTER sort_order,
    ADD COLUMN IF NOT EXISTS created_by INT DEFAULT NULL AFTER published_at,
    ADD COLUMN IF NOT EXISTS updated_by INT DEFAULT NULL AFTER created_by;

ALTER TABLE projects
    ADD UNIQUE KEY IF NOT EXISTS uq_projects_slug (slug),
    ADD INDEX IF NOT EXISTS idx_projects_category (category),
    ADD INDEX IF NOT EXISTS idx_projects_public_listing (status, is_featured, sort_order, created_at);
...
ALTER TABLE inquiries
    ...
    ADD COLUMN IF NOT EXISTS status ENUM('new', 'read', 'replied', 'archived') NOT NULL DEFAULT 'new' AFTER message,
    ADD COLUMN IF NOT EXISTS source VARCHAR(50) NOT NULL DEFAULT 'website' AFTER status,
    ADD COLUMN IF NOT EXISTS admin_note TEXT DEFAULT NULL AFTER source,
    ADD COLUMN IF NOT EXISTS replied_at DATETIME DEFAULT NULL AFTER admin_note,
    ADD COLUMN IF NOT EXISTS resolved_at DATETIME DEFAULT NULL AFTER replied_at,
    ADD COLUMN IF NOT EXISTS ip_address VARCHAR(45) DEFAULT NULL AFTER resolved_at,
    ADD COLUMN IF NOT EXISTS user_agent VARCHAR(255) DEFAULT NULL AFTER ip_address,
    ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP AFTER created_at;

ALTER TABLE inquiries
    ADD INDEX IF NOT EXISTS idx_inquiries_status_created_at (status, created_at),
    ADD INDEX IF NOT EXISTS idx_inquiries_email (email);
...
UPDATE projects p
INNER JOIN project_categories c ON c.name COLLATE utf8mb4_general_ci = p.category
SET p.project_category_id = c.id
WHERE p.project_category_id IS NULL;
